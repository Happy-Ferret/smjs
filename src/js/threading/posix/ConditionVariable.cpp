...
bool ConditionVariable::wait(Mutex& mutex, uint64_t usec) {
    ...
    uint64_t nsec = usec * NSEC_PER_USEC;
    struct timespec ts;
    int r;
    ...
    r = pthread_cond_timedwait_relative_np(ptCond, ptMutex, &ts);
    ...
    // ts.tv_nsec may not exceed NSEC_PER_SEC - 1.
    nsec += ts.tv_nsec;
    ts.tv_sec += nsec / NSEC_PER_SEC;
    ts.tv_nsec = nsec % NSEC_PER_SEC;

    r = pthread_cond_timedwait(ptCond, ptMutex, &ts);
#endif
